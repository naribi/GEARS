...
1. For the Sentinel-2 image, calculate the spectral indices according to the equations provided in the following table:

![Figure 11. Spectral Indices](Spectral-Indices.PNG)

2. Search for a cloud-free Sentinel-2 image from May, July and September 2018 collected over Litchfield National Park (Litchfield is located south of Darwin, near the town of Batchelor, Northern Territory, Australia).
3. Calculate NDVI for each of the scenes and load them into the map view.
4. Inspect how NDVI varies spatially across each image, and explore how patterns in NDVI vary according to time of year.
5. Search for a cloud-free Landsat 8 image (USGS Landsat 8 Surface Reflectance Tier 1) from May, July and September 2018 collected over Litchfield National Park.
6. Remember that the band position of RED and NIR wavelengths might differ between different sensors. For Landsat 8, the metadata property for cloud cover is 'CLOUD_COVER'.
7. Compare NDVI values attained from the two sensors and think about why they might differ.
